...
import (
	...
	// cosmwasm-testing
	"github.com/apollodao/test-tube/neutron-test-tube/result"
	"github.com/apollodao/test-tube/neutron-test-tube/testenv"
)
...
func InitTestEnv() uint64 {
	...
	env.App = testenv.SetupApp()
	env.ParamTypesRegistry = *testenv.NewParamTypeRegistry()

	env.StakingKeeper = stakingkeeper.NewKeeper(
		env.App.AppCodec(),
		env.App.GetKey(stakingtypes.StoreKey),
		env.App.AccountKeeper,
		env.App.BankKeeper,
		env.App.GetSubspace(stakingtypes.ModuleName),
	)
